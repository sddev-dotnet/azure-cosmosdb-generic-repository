    ...
    public class GenericRepositoryTests
    {
        ...
        public static async Task Cleanup()
        {
            if (_client is not null)
            {
                try
                {
                    var database = _client.GetDatabase(_cosmos.Value.DefaultDatabaseName);

                    // Best-effort cleanup for integration tests (containers might not exist depending on test execution)
                    await database.GetContainer("TestObject").DeleteContainerAsync();
                    await database.GetContainer("AnotherTestObject").DeleteContainerAsync();
                    await database.GetContainer("BaseTest").DeleteContainerAsync();
                    await database.GetContainer("Testing").DeleteContainerAsync();
                }
        ...
        //{
        //    // Arrange
        //    var repo = new GenericRepository<TestAuditableObject>(_client, _factory.CreateLogger<GenericRepository<TestAuditableObject>>(), _cosmos, "Testing");
